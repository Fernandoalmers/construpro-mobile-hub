...
-- First drop functions that have parameter name conflicts
DROP FUNCTION IF EXISTS public.mask_cpf(text);
DROP FUNCTION IF EXISTS public.mask_email(text);
DROP FUNCTION IF EXISTS public.mask_phone(text);
DROP FUNCTION IF EXISTS public.migrate_missing_orders_to_pedidos();
DROP FUNCTION IF EXISTS public.validate_order_has_items(uuid);
DROP FUNCTION IF EXISTS public.check_order_integrity();
DROP FUNCTION IF EXISTS public.get_or_create_reference_id(uuid);
DROP FUNCTION IF EXISTS public.sync_order_status_with_reference(uuid, text);
DROP FUNCTION IF EXISTS public.sync_order_to_pedidos(uuid);
DROP FUNCTION IF EXISTS public.sync_pedidos_status_to_orders(uuid, text);

-- Fix update functions (triggers) - these can be updated directly
...
-- Recreate data masking functions with secure search_path
CREATE FUNCTION public.mask_cpf(cpf_input text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
...
CREATE FUNCTION public.mask_email(email_input text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
...
CREATE FUNCTION public.mask_phone(phone_input text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
