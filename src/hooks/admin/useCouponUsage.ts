...
export const useCouponUsage = (couponId?: string) => {
  ...
  const fetchCouponUsage = async (id: string) => {
    ...
    try {
      ...
      // Use native Supabase query with relationships
      ...
      const enrichedUsage: CouponUsageDetail[] = usageRecords.map(usage => {
        ...
        // Collect vendor names from order items
        ...
        console.log(`[useCouponUsage] Processing usage ${usage.id} with vendors:`, vendorNamesList);
