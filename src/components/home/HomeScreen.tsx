...
const HomeScreen: React.FC = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('points_transactions').select('*').eq('user_id', user.id).order('data', {
        ascending: false
      });
      if (error) {
  ...
  if (error) {
    return <ErrorState title="Erro ao carregar dados" message={error} onRetry={() => window.location.reload()} />;
  }
  ...
  return <div className="flex flex-col bg-gray-100 min-h-screen pb-20">
      ...
      <div className="p-6 pt-12 rounded-b-3xl bg-construPro-blue">
        <div className="flex justify-between items-center mb-4">
          ...
          <Avatar src={profile?.avatar || undefined} alt={userName} fallback={userName} size="lg" className="border-2 border-white" onClick={() => navigate('/profile')} />
        </div>
        
        <Card className="p-4 mb-4">
          <div className="flex justify-between items-center mb-2">
            <p className="text-gray-600">Seu saldo</p>
            <CustomButton variant="link" onClick={() => navigate('/profile/points-history')} className="flex items-center text-construPro-blue p-0">
      ...
      <div className="px-6 -mt-6">
        <MonthlyLevelProgress currentMonth={currentMonth} levelInfo={levelInfo} />
      </div>
      ...
      <div className="p-6 pt-2">
        ...
        <div className="space-y-4">
          {promoItems.map(item => <Card key={item.id} className={`p-4 border-l-4 border-construPro-orange ${item.color}`}>
