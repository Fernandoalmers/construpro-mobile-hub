        ...
        if (itemsError) {
          ...
          return { 
            ...
            cliente: order.cliente && typeof order.cliente === 'object' ? {
              id: order.cliente.id ? order.cliente.id as string : '',
              vendedor_id: vendorProfile.id,
              usuario_id: order.usuario_id,
              nome: order.cliente.nome ? order.cliente.nome as string : 'Cliente',
              telefone: order.cliente.telefone ? order.cliente.telefone as string : '',
              email: order.cliente.email ? order.cliente.email as string : '',
              total_gasto: 0
            } : undefined
          };
        }
